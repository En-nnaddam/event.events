...
function EventStatusForm({ event }: { event: AdminEventRow }) {
  ...
  return (
    <form action={updateEventStatus}>
      ...
      <button
        type="submit"
        className="inline-flex h-8 items-center justify-center rounded-md border border-border px-3 text-xs font-medium transition hover:bg-muted focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/40"
      >
...
function DeleteEventForm({ event }: { event: AdminEventRow }) {
  return (
    <form action={deleteEvent}>
      <input type="hidden" name="event_id" value={event.id} />
      <button
        type="submit"
        className="inline-flex h-8 items-center justify-center rounded-md border border-destructive/30 px-3 text-xs font-medium text-destructive transition hover:bg-destructive/10 focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-destructive/20"
      >
        Delete
      </button>
    </form>
...
export default async function AdminEventsPage({ searchParams }: AdminEventsPageProps) {
  ...
  const activeStatus = params.status === "published" || params.status === "archived" ? params.status : "all"
  ...
  const [{ data: events }, { count: publishedEvents }, { count: archivedEvents }] = await Promise.all([
    query.returns<AdminEventRow[]>(),
    ...
    <main className="min-h-svh bg-background p-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-normal">Events management</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              Create, edit, publish, archive, and delete events shown on the public homepage.
            </p>
          </div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-3">
          <div className="rounded-lg border border-border bg-card p-4 shadow-sm">
            ...
            <p className="mt-2 text-3xl font-semibold">{(publishedEvents ?? 0) + (archivedEvents ?? 0)}</p>
          </div>
          <div className="rounded-lg border border-border bg-card p-4 shadow-sm">
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {["all", "published", "archived"].map((status) => (
            <Link
              key={status}
              href={status === "all" ? "/admin/events" : `/admin/events?status=${status}`}
              className={`inline-flex h-8 items-center justify-center rounded-md border px-3 text-sm font-medium transition ${activeStatus === status
        ...
        <div className="mt-6 grid gap-4">
          {events?.length ? (
            events.map((event) => (
              <article key={event.id} className="grid gap-4 rounded-lg border border-border bg-card p-4 shadow-sm lg:grid-cols-[160px_1fr_auto]">
                <div
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {event.categories?.name ? <StatusPill status={event.categories.name} /> : null}
                    <span className="text-xs text-muted-foreground">{formatEventDate(event.starts_at)}</span>
                  </div>
            ...
            <div className="rounded-lg border border-border bg-card px-5 py-12 text-center shadow-sm">
              <h2 className="text-lg font-semibold tracking-normal">No events found</h2>
