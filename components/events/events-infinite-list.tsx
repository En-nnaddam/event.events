...
export function EventsInfiniteList() {
  ...
  const onlyPublishedEvents = useCallback<SupabaseQueryHandler>((query) => {
    return query.eq("status", "published").order("starts_at", { ascending: true })
  }, [])

  const { data, error, fetchNextPage, hasMore, isFetching, isLoading, isSuccess } =
  ...
  return (
    <div className="grid gap-5">
      ...
      {!hasMore && data.length > 0 ? (
        <p className="py-4 text-center text-sm text-muted-foreground">You have reached the end.</p>
      ) : null}
    </div>
  )
}
